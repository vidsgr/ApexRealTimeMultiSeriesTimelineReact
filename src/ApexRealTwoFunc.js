...
function getNewSeries(baseval, yrange) {
  ...
  for (var j = 0; j < 2; j++) {
    ...
    //series[j].data[i].y = 0
    series[j].data = series[j].data.slice(
      series[j].data.length - 60,
  ...
  xaxis: {
    ...
    labels: {
      ...
      formatter: function(value) {
        if (value.split(" ")[0].split(":")[2] % 10 == 1) return value;
        else return "";
      },
      minHeight: 100,
      maxHeight: 150
...
function intervals() {
  window.setInterval(() => {
    ...
    ApexCharts.exec("realtime3", "updateSeries", series);
  }, 1000);
}

function ApexRealTwoFunc() {
  const [options, setOptions] = useState(()=>{
  getDayWiseTimeSeries(new Date("09/23/2019 22:44:00").getTime(), 60, {min: 10,max: 90});
  return options0;
  });
...
export default ApexRealTwoFunc;
